# PowerShell script to set all ECS service desired_count to 0
# This script will scale down all services in all clusters to 0 tasks
...
Write-Host "ECS Service Scale Down Script" -ForegroundColor Green
...
try {
    ...
    foreach ($cluster in $clusters) {
        ...
        foreach ($service in $services) {
            ...
            if ($status -eq "ACTIVE" -and $currentDesiredCount -gt 0) {
                $servicesToUpdate += @{
                    Cluster = $cluster
                ...
                $totalServices++
            } elseif ($currentDesiredCount -eq 0) {
                Write-Host "      Already at 0 - skipping" -ForegroundColor Yellow
            } else {
    ...
    if ($servicesToUpdate.Count -eq 0) {
        Write-Host ""
        Write-Host "No services need to be updated (all are already at 0 or inactive)" -ForegroundColor Green
        exit 0
    }
    ...
    foreach ($service in $servicesToUpdate) {
        Write-Host "  $($service.Cluster)/$($service.ServiceName) : $($service.CurrentDesiredCount) -> 0" -ForegroundColor White
    }
    ...
    if (-not $Force) {
        Write-Host ""
        Write-Host "This will scale down $($servicesToUpdate.Count) service(s) to 0 tasks." -ForegroundColor Yellow
    ...
    foreach ($service in $servicesToUpdate) {
        ...
        $updateResult = aws ecs update-service --cluster $service.Cluster --service $service.ServiceName --desired-count 0 --output json 2>&1
    ...
    if ($successCount -gt 0) {
        Write-Host ""
        Write-Host "Note: Services are being scaled down. It may take a few minutes for tasks to fully terminate." -ForegroundColor Yellow
